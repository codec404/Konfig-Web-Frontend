...
import { getServiceConfigs, deleteConfig } from '../api/configs'
...
export default function ConfigList({ serviceName }: ConfigListProps) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['configs', serviceName, page],
    queryFn: () => getServiceConfigs(serviceName, limit, page * limit),
  })

  const deleteMutation = useMutation({
    mutationFn: deleteConfig,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['configs', serviceName] })
    },
  })

  function handleDelete(configId: string, e: React.MouseEvent) {
    e.stopPropagation()
    if (!confirm('Delete this config? This action cannot be undone.')) return
    ...
    <div>
      ...
      {configs.length === 0 ? (
        <div className="empty-state">
          ...
          <div className="empty-state-desc">Upload the first config for this service.</div>
        </div>
      ) : (
